...
- 中国科学技术大学陈恩红教授博士生
- 在实验室担任智慧教育方向研究组的组长
- 主要研究兴趣和方向包括：教育数据挖掘，自然语言处理，强化学习等
- 自我评价：
  - 注重研究的实际价值
    - 围绕教育场景，从人的实际需要出发，深耕资源理解、认知诊断、教育推荐等领域
    - 在教育数据挖掘领域有较高的影响力，Google Scholar 总引用量 820, h-index 11
  - 有着较强的 0-1 实现能力
...
- 在刘淇老师、黄振亚老师指导下成立教育组，带领组员探索教育数据挖掘相关领域，如**试题理解**、**学生建模**、**自适应学习和推荐**等，指导小组成员发表 20 余篇学术论文。
...
- 背景：
  - 高校编程教学数字化程度不够，好的编程教学很需要一个完整的在线学习系统。
  - 现有的编程教学平台场景简单（类似 OJ），且十分依赖用户自驱。
  - 教育相关的研究很需要真实数据和落地验证，才能实际促进教学质量提升。
- 贡献：
  - 从零开始搭建智能在线编程平台 CODIA，使用 Vue + Node.js 设计并实现了初始网站架构和基于 GraphQL 的业务 API，并基于 gRPC 和 Kafka 实现可扩展的微服务架构。
  - 制定同时面向学生和教师的产品策略，为学生提供更好的编程学习体验（如在线 IDE、可视化能力诊断等），为教师提供更好的教学辅助（如题目设置、学情分析等）。
  - 将教育组的**认知诊断**和**自适应推荐**等相关研究应用到平台上，指导团队成员将知识追踪和试题推荐模型封装为 gRPC 服务，促进研究的真实场景验证。
  - 搭建基于 GitLab 和 Kubernetes 的 DevOps+MLOps 平台，用于 CODIA 的功能开发和持续部署，提高团队工作效率、降低协作成本。
  - 已多次作为中科大多门编程和算法课程的教学平台，为相关专业的教师和学生提供更好的学习体验。

---

# CODIA 展示

<div class="mt-4 grid place-items-center grid-cols-2 gap-y-8">
  <img
  ...
  />
  <img
    src="/imgs/resume/codia_pack.png"
    alt="CODIA pack"
    class="h-42 outline outline-gray-300"
  />
  <img
    src="/imgs/resume/codia_teacher.png"
    alt="CODIA teacher"
    class="h-40 outline outline-gray-300"
  />
  <img
...
- 基于考试中心真实场景和需求，探索新的研究课题，例如**试卷分割**、**试题查重**、**相似题推荐**等，相关成果用于开发考试中心内部工具，以及实验室智慧教育知识图谱平台 LUNA。
...
- 基于基础 NLP 技术开展**题库建设**、**公式提取**和**相似题检索**等任务，并参与重构讯飞内部海量试题库以及相似题检索系统。
- 使用深度学习方法进行**试题理解**和**难度估计**等任务，参与开发和优化相关模型，在高考试题难度预测准确性和稳定性指标上超过领域专家。
- 在知识追踪任务中引入试题题面文本信息，实现了了一种试题增强的**知识追踪**方法，大幅提升学生得分预测的效果。
...
- QuesNet 贡献：
  ...
  - 引入 domain-specific task 多任务学习
    - token 层面：holed language model
    - 试题层面：题面 $\to$ 题解 (domain-specific)
...
# QuesNet $\to$ QuesNet LLM

- 近期在实验室筹备和组织教育领域大模型工作
- 现有基础：
  - 教育组成立了 EduNLP 团队，对数据清洗、分析、分类等基础任务
  - 近年来不断扩充试题库，试题库现包含约 2600w 道全学科试题
  - 团队在试题理解和预训练表征方向已有数篇研究工作发表
- QuesNet LLM 技术路线：
  - 数据团队：负责清洗、筛选、格式标准化等任务，建立高质量的试题语料库和 instruction 数据集
  - 模型团队：负责基于 LLaMA 搭建和调优 architecture、训练和微调 pipeline 搭建、分布式训练等
  - benchmark 团队：负责建立教育领域标准化 benchmark 任务和数据集，主要包括知识点标注、难度预估、自动解题等经典任务

---

# 代表性成果：EKT 和 DTransformer

- 任务：知识追踪
- 目标：根据学生做题记录诊断学生在每个时刻的知识掌握状态
- EKT 贡献：
  - **第一个**在 KT 中引入试题文本的工作（之前的工作均基于知识点 embedding）
  - 通过对文本 attention 聚合最相关的历史记录
  - 通过对知识矩阵 attention 形成每个知识点上的追踪
- DTransformer 贡献：
  - 解决诊断结果不稳定、对学生作答记录过于敏感的问题
...
- 任务：在线教学系统中，为学生自适应地推荐试题
- 多目标：区别于传统推荐，自适应试题推荐需要更多考虑学生实际需求、同时兼顾多个学习目标
  - 平衡回顾和扩展（避免死磕一个知识点）
  - 试题难度平滑上升（保证合理的学习梯度）
  - 保持学生参与度（促进学习的持续性）
- 贡献：
...
<div class="mt-2 grid place-items-center grid-cols-1">
  <img
    src="/imgs/resume/dre.png"
    alt="DRE model"
    class="h-36 outline-gray-300"
  />
</div>
...
- 跟进 LLM 最新进展
- 对 LLM 的理解：
  - LLM 基本具备运用自然语言的能力，有潜力改变人机交互方式
  - LLM 类似一种基于自然语言指令的 CPU，为 LLM 增加记忆、反馈控制、外部输入等，类似围绕 CPU 建立起一套基于自然语言的通用计算机
  - 参考云计算的发展历程，未来基于 LLM 提供的通用云智能服务会更加普遍
- 从开源 LLM 源码学习实现（LLaMA, Alpaca, ColossalChat）
...
- 整体效果
  - 提高数据质量：爬取高质量来源，数据精细化预处理，设计标注任务
  - 模型角度：GLM 多种范式训练，增加模型容量，增长模型记忆
  - 评价指标角度：用更好的评价指标监控模型进展（如引入自评价）
- 推理能力和代码能力
  - 数学题：引入数学解题步骤数据，数据标准化，多模态数据
  - 代码：筛选和预处理，增广高质量代码，构造精细任务数据集（如测试代码）
- 效率角度
  - 训练/微调效率：attn 加速、编译优化技术、分布式训练技术、低精度训练、少量参数微调
  - 推理速度：参数量化，多终端优化（陈天奇 WebGPU，苹果 M 芯片）
  - 迭代效率：建立 pipeline，快速封装验证发布模型，类似 huggingface 的模型和数据集发布平台
- 适用范围
  - 发布不同量级的大模型，小量级专注特定领域，发挥小量级的优势（学习 google 思路）
  - 针对检索增强、记忆增强、多轮迭代等应用场景优化模型（如构造相应数据集）
...
- 代码是对整个算法的最完备和准确的描述，读代码能够最直接、完整地掌握算法
- 最近的一次读代码经历：理解了 PPO 算法，熟悉其实现（核心在于 experience buffer 中放了什么）
- 从代码中学习一些 best practice
  - 数据预处理，参数初始化等
  - 运用 einops 等工具更好地表达模型结构/计算
  - 效率优化如分布式训练
- 从代码中产生新的理解
  - CL loss 的实现启发：从固定几个类别中选出正确分类 $\to$ 从任意一些候选中选出最相关的候选
